...
public class BoardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("writeform".equals(action)) {
			...
			originVo.setNo(Integer.parseInt(no));
			originVo.setPassword(password);
			
			if(new BoardDao().findByNo(no).getPassword().equals(password)) {	
				new BoardDao().delete(originVo);
				WebUtil.redirect(request.getContextPath() + "/board?p=1", request, response);
			}else {
				//비밀번호 틀렸을때
			...
			originVo.setG_no(Integer.parseInt(g_no));
			originVo.setO_no(Integer.parseInt(o_no));
			originVo.setDepth(Integer.parseInt(depth));
